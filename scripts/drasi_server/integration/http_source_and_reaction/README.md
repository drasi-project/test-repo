# Drasi Server HTTP Integration Test

This is a self-contained integration test that uses the E2E Test Framework to test Drasi Server. The test validates the complete end-to-end data flow using HTTP source and reaction endpoints.

## Purpose

This test validates:
- **HTTP Source Integration**: Data ingestion via HTTP on port 9000
- **Continuous Query Processing**: Cypher query execution with real-time updates
- **HTTP Reaction Delivery**: Query results pushed via HTTP on port 9001
...
Use this test to verify that changes to Drasi Server and Drasi Core haven't broken the HTTP integration pathways.
...
- Branch: `main`
- File: [`dev_repo/drasi_server/integration/building_comfort_http.test`](https://github.com/drasi-project/test-repo/blob/query-host/dev_repo/drasi_server/integration/building_comfort_http.test)

The test definition is specified in `test-service-config.yaml` under the `test_id` field (`building_comfort_http`). The Test Service will fetch this file from GitHub at runtime, so you don't need to manually download it.
...
```bash
cd scripts/drasi_server/integration/http_source_and_reaction
...
Expected output:
```
Starting Drasi Server Test - HTTP (Debug Mode)
================================================
...
1. Check port availability (8080, 9000, 9001)
...
| 9000 | HTTP Source | Incoming | E2ETF → Drasi (data changes) |
| 9001 | HTTP Reaction | Outgoing | Drasi → E2ETF (query results) |
...
```
test_data_store/test_runs/github_dev_repo.building_comfort_http.test_run_001/sources/facilities-db/source_output/source_events_*.log
```

Source performance metrics are saved to:
```
test_data_store/test_runs/github_dev_repo.building_comfort_http.test_run_001/sources/facilities-db/source_output/performance_metrics/performance_metrics_*.json
```
...
```json
{
  ...
  "test_run_source_id": "github_dev_repo.building_comfort_http.test_run_001.facilities-db"
}
```
...
```
test_data_store/test_runs/github_dev_repo.building_comfort_http.test_run_001/reactions/building-comfort/reaction_output/reaction_events_*.log
```

Reaction performance metrics are saved to:
```
test_data_store/test_runs/github_dev_repo.building_comfort_http.test_run_001/reactions/building-comfort/reaction_output/performance_metrics/performance_metrics_*.json
```
...
```json
{
  ...
  "test_run_reaction_id": "github_dev_repo.building_comfort_http.test_run_001.building-comfort",
...
4. Verify the test file exists at: `dev_repo/drasi_server/integration/building_comfort_http.test`
