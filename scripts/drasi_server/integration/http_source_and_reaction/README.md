...
This is a self-contained integration test for Drasi Server that validates the complete end-to-end data flow using HTTP source and reaction endpoints.
...
Use this test to verify that changes to Drasi Server haven't broken the core HTTP integration pathways.
...
- id: facilities-db
  ...
  host: 0.0.0.0
  port: 9000                    # HTTP source endpoint
```
...
`dev_repo/drasi_server/integration/building_comfort_http.test`
...
- **Source Dispatcher**: HTTP to localhost:9000
- **Reaction Handler**: HTTP on 0.0.0.0:9001
...
test_run_host:
  test_runs:
    - test_id: building_comfort_http    # References the test file
...
```
test_data_store/test_runs/github_dev_repo.building_comfort_http.test_run_001/reactions/building-comfort/output_log/performance_metrics/performance_metrics_*.json
```
...
```bash
grep "Processing HTTP event" drasi-server-debug.log | head -10
```
...
**Common issues:**
- **HTTP connection failed**: Check if ports 9000/9001 are actually listening
...
## Key Differences from gRPC Test

This HTTP test differs from the gRPC test in the following ways:

1. **Transport Protocol**: Uses HTTP/1.1 instead of gRPC (HTTP/2)
2. **Ports**: Uses 9000/9001 instead of 50051/50052
3. **Source Configuration**: HTTP source vs gRPC source
4. **Reaction Configuration**: HTTP reaction endpoint vs gRPC reaction
5. **Message Format**: JSON over HTTP vs Protocol Buffers over gRPC

## Related Tests

- `../grpc_source_and_reaction/` - Tests standard gRPC source and reaction
